...
import static com.tobe.healthy.common.FileUpload.FILE_MAXIMUM_UPLOAD_SIZE;
...
public class LessonHistoryCommandService {
    ...
    public CommandRegisterLessonHistoryResult registerLessonHistory(CommandRegisterLessonHistory request, Long trainerId) {
        ...
        LessonHistory lessonHistory = LessonHistory.register(request.getTitle(), request.getContent(), student, trainer, schedule);
    ...
    public CommandUpdateLessonHistoryResult updateLessonHistory(Long lessonHistoryId, CommandUpdateLessonHistory request, Long trainerId) {
            ...
            for (int idx = 0; idx < requestFiles.size(); idx++) {
                ...
                if (existingIdx != -1) {
                    ...
                    if (file.getFileUrl().startsWith(S3_DOMAIN)) {
                        ...
                        LessonHistoryFiles newFile = new LessonHistoryFiles(result.getFileUrl(), idx + 1, lessonHistory.getTrainer(), lessonHistory);
                        savedFiles.add(newFile);
                    } else if (file.getFileUrl().startsWith(CDN_DOMAIN)) {
                        LessonHistoryFiles newFile = new LessonHistoryFiles(file.getFileUrl(), idx + 1, lessonHistory.getTrainer(), lessonHistory);
                        savedFiles.add(newFile);
                    }
                }
            }
    ...
    public CommandRegisterCommentResult registerLessonHistoryComment(Long lessonHistoryId, CommandRegisterComment request, CustomMemberDetails member) {
        Member findMember = findMember(member.getMemberId());

        LessonHistory lessonHistory = lessonHistoryRepository.findById(lessonHistoryId, member.getMemberId(), member.getMemberType());
        if (lessonHistory == null) {
            throw new CustomException(ErrorCode.LESSON_HISTORY_NOT_FOUND);
        }
        ...
        List<LessonHistoryFiles> files = registerFile(request.getUploadFiles(), findMember, lessonHistory, lessonHistoryComment);
        ...
        if (!member.getMemberId().equals(lessonHistory.getTrainer().getId())) {
            sendNotification(
                    ...
                    "https://main.to-be-healthy.shop/trainer/manage/" + lessonHistory.getStudent().getId() + "/log/" + lessonHistory.getId(),
                    lessonHistory.getStudent().getId(),
    ...
    public CommandRegisterReplyResult registerLessonHistoryReply(Long lessonHistoryId, Long lessonHistoryCommentId,
        ...
        LessonHistoryComment entity = new LessonHistoryComment(order, request.getContent(), findMember, lessonHistory, parentComment);
    ...
    public CommandUpdateCommentResult updateLessonHistoryComment(Long lessonHistoryCommentId, CommandUpdateComment request, CustomMemberDetails member) {
        LessonHistoryComment comment = lessonHistoryCommentRepository.findLessonHistoryCommentWithFiles(lessonHistoryCommentId, member.getMemberId());
        if (comment == null) {
            ...
            for (int idx = 0; idx < requestFiles.size(); idx++) {
                ...
                if (existingIdx != -1) {
                    ...
                    if (file.getFileUrl().startsWith(S3_DOMAIN)) {
                        String tempUrl = file.getFileUrl().replace(S3_DOMAIN, "");
                        CommandUploadFileResult result = moveDirTempToOrigin("origin/lesson-history/", tempUrl, idx + 1);
                        LessonHistoryFiles newFile = new LessonHistoryFiles(result.getFileUrl(), idx + 1, comment.getWriter(), comment.getLessonHistory(), comment);
                        savedFiles.add(newFile);
                    } else if (file.getFileUrl().startsWith(CDN_DOMAIN)) {
                        LessonHistoryFiles newFile = new LessonHistoryFiles(file.getFileUrl(), idx + 1, comment.getWriter(), comment.getLessonHistory(), comment);
                        savedFiles.add(newFile);
                    }
                }
            }
    ...
    private List<LessonHistoryFiles> registerFile(List<CommandUploadFileResult> uploadFiles, Member member,
        ...
        for (int idx = 0; idx < uploadFiles.size(); idx++) {
            ...
            if (uploadFile.getFileUrl().startsWith(S3_DOMAIN)) {
                ...
                CommandUploadFileResult result = moveDirTempToOrigin("origin/lesson-history/", tempUrl, idx + 1);
                LessonHistoryFiles file = new LessonHistoryFiles(result.getFileUrl(), result.getFileOrder(), member, lessonHistory, lessonHistoryComment);
                files.add(file);
            }
        }
    ...
    private LessonHistoryComment registerComment(int order, CommandRegisterComment request, Member findMember, LessonHistory lessonHistory) {
    ...
    private List<LessonHistoryFiles> registerFiles(List<CommandUploadFileResult> uploadFiles, Member member, LessonHistory lessonHistory) {
        checkMaximumFileCount(uploadFiles.size());
        ...
        for (int idx = 0; idx < uploadFiles.size(); idx++) {
            ...
            if (uploadFile.getFileUrl().startsWith(S3_DOMAIN)) {
                String tempUrl = uploadFile.getFileUrl().replace(S3_DOMAIN, "");
                CommandUploadFileResult result = moveDirTempToOrigin("origin/lesson-history/", tempUrl, idx + 1);
                LessonHistoryFiles file = new LessonHistoryFiles(result.getFileUrl(), result.getFileOrder(), member, lessonHistory);
                files.add(file);
            }
        }
