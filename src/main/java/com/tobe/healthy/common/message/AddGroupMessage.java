...
//        jsonObject.addProperty("imageId", "[IMAGE_ID를 입력하세요]");
		...
		Call<AddMessageListModel> api = APIInit.getAPI()
			.addGroupMessage(APIInit.getHeaders(), "[Group ID를 입력하세요]",
				new MessageList(jsonString));
		api.enqueue(new Callback<AddMessageListModel>() {
			@Override
			public void onResponse(Call<AddMessageListModel> call,
				Response<AddMessageListModel> response) {
