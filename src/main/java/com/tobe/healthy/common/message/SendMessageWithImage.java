...
//            System.out.println(new File("src/image/testImage.jpg").getAbsoluteFile());
		...
		Call<ImageResult> api = APIInit.getImageAPI()
			.createImage(APIInit.getHeaders(), new ImageModel(result));
		api.enqueue(new Callback<ImageResult>() {
			...
			public void onResponse(Call<ImageResult> call, Response<ImageResult> response) {
				...
				if (response.isSuccessful()) {
					...
					Message message = new Message("[수신번호를 입력하세요]", "[발신번호를 입력하세요]",
						"[전송할 문자를 입력하세요]", "[전송할 제목을 입력하세요]", imageId);
					Call<MessageModel> sendApi = APIInit.getAPI()
						.sendMessage(APIInit.getHeaders(), message);
