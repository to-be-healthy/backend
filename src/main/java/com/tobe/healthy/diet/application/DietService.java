...
public class DietService {
	...
	public DietDto addDiet(Member member, DietAddCommand command) {
		...
		if (uploadDays.contains(command.getEatDate()))
	...
	private void deleteOldFiles(Diet diet, DietUpdateCommand command) {
		...
		if (command.getBreakfastFile() != null && !command.isBreakfastFast())
			oldFileNames.remove(getFileName(command.getBreakfastFile()));
		if (command.getLunchFile() != null && !command.isLunchFast())
			oldFileNames.remove(getFileName(command.getLunchFile()));
		if (command.getDinnerFile() != null && !command.isDinnerFast())
			oldFileNames.remove(getFileName(command.getDinnerFile()));
