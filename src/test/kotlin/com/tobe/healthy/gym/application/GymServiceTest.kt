...
class GymServiceTest(
    ...
    "헬스장을 등록한다" {
        // given
        val request = CommandRegisterGym(
            name = "건강해짐 화정점"
        )

        // when
        val response = gymCommandService.registerGym(request)
    ...
    "내 헬스장으로 등록할 때 가입 번호가 틀리면 예외가 발생한다" {
        ...
        val message = shouldThrow<CustomException> {
            gymCommandService.selectMyGym(findGym?.id!!, CommandSelectMyGym(joinCode = "12345"), trainer.id)
        }.message
